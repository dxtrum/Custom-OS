...
** Name:        __strtok
**
** Description: creates a token from a larger string
**
** @param tok   The destination buffer for the token
** @param src   The source buffer
** @param ch    The delimeter character
**
** @return 	count that the source has moved
**	   	allows the function to be used in loops 
**
** NOTE:  May not correctly deal with overlapping buffers.  Assumes
** tok is large enough to hold the resulting string.
*/
int __strtok( register char *tok, register const char *src, char ch ){

    int i = 0;

    //if src starts with ch move until it doesn't
    while (*src == ch){
	++src;
	++i;
    }

    while( (*tok++ = *src++) && *src != ch){
        ++i;
    }
    *tok = '\0';
